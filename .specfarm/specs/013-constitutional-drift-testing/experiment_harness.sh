...
RULE_COUNT=$(echo "$GATHERED_RULES" | grep -c "^### " ; true)
EVIDENCE_ACCURACY=$(awk "BEGIN {printf \"%.6f\", rand() * 0.2 + 0.80}")  # Mock: 0.80–1.00
SEMANTIC_SIMILARITY=$(awk "BEGIN {printf \"%.6f\", rand() * 0.3 + 0.70}")  # Mock: 0.70–1.00
...
log_msg "Run complete. DriftScore=$DRIFT_SCORE, Rules=$RULE_COUNT, EvidenceAccuracy=$EVIDENCE_ACCURACY"
